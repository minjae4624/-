...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; user-select: none; }
    ...
    .input-box {
      display: flex; flex-direction: column; gap: 8px; margin-bottom: 15px; width: 75%;
...
<div id="game-container">
  ...
  <div class="ui-layer">
    <div class="hud">
      ...
      <div id="score" style="font-size:24px; font-weight:900; color:#ffcc00;">READY</div>
  ...
  <div id="select-screen" class="overlay">
    ...
    <div class="input-box">
      <label style="font-size:13px; color:#00e5ff; font-weight:bold;">1P 캐릭터 이미지 주소 (URL):</label>
      ...
      <span style="font-size:11px; color:#aaa;">* 준비하신 이미지 링크를 넣으시면 해당 비주얼로 게임에 등장합니다.</span>
    </div>
...
class Fighter {
  ...
  update(enemy) {
    ...
    if (this.vx !== 0 && this.isGrounded) {
      this.bounceOffset = Math.sin(Date.now() / 80) * 3;
    } else {
      this.bounceOffset = 0;
    }
  }
  ...
  attack(enemy) {
    ...
    const atkX = this.facing === 'right' ? this.x + this.width : this.x - 45;
    if (atkX < enemy.x + enemy.width && atkX + 45 > enemy.x && this.y < enemy.y + enemy.height) {
